...
| Comtrade (funnel) | `ComtradeProvider` | `COMTRADE_API_KEY` + `COMTRADE_OFFLINE=0` | ✅ / ⚠️ | Robust: broad `except` → cache/fixture fallback (I1), correct `Ocp-Apim-Subscription-Key`. **No call budget / throttle / circuit-breaker** — the prompt requires ≤150 calls/analysis and says live Comtrade should use the engine's `silk_data_layer`. This simpler adapter has neither; add budgeting before opening the tap. |
...
| Sending (cold) | `SmartleadSendingProvider` | `SMARTLEAD_API_KEY` | ⚠️ **High** | `SMARTLEAD_URL = .../api/v1/email/reply` is the *reply-to-an-existing-email* endpoint, and the flat one-shot `send` payload doesn't match Smartlead's **campaign-based** model (add leads to a campaign; Smartlead sends per sequence). **This adapter likely needs a rework against Smartlead's real API before any live send.** Not guessed at — money/sending path. |
...
matching `ComtradeProvider`'s existing precedent. The **send** adapters were left
untouched on purpose: broadening their error handling could turn a real send into
`accepted=False` and trigger a duplicate — a money/sending risk not worth taking
...
4. **Smartlead/Instantly + per-factory OAuth** → configure **SPF + DKIM + DMARC on a dedicated subdomain FIRST**, then warm-up ramp, then a supervised pilot campaign. (Resolve the Smartlead-adapter flag before this step.)
