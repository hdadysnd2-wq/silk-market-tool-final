...
- **`SECRET_KEY`** is generated by the script and set on `api`/`worker`/`beat`
  (it signs auth tokens and derives the mailbox token-encryption key). The **`web`**
  service must carry the **same** key — the Next.js server *verifies* the session
  JWT, and without a matching key every logged-in page fails verification and
  bounces back to `/login` (a login loop). The script wires `web`'s key as a
  Railway reference (`SECRET_KEY=${{api.SECRET_KEY}}`) so the four never drift.
  Rotate it for production and, ideally, set an explicit `TOKEN_ENCRYPTION_KEY`
...
### Login "loops": it accepts the password, then returns to `/login` with no error

The `web` service is missing (or has a mismatched) `SECRET_KEY`. The Next.js
server *verifies* the session JWT it just minted; with no matching key
`verifyToken` returns null and every authenticated page redirects back to
`/login`.

This is **service configuration, not code** — merging a fix or redeploying the
same code does not change it, and re-running the deploy script **skips services
that already exist**. Fix an already-running deployment by hand:

1. Railway → the **`web`** service → **Variables** → add:
   `SECRET_KEY=${{api.SECRET_KEY}}`
   (a reference to the api's key — no need to copy the literal value; it always
   matches and follows a rotation).
2. Redeploy `web` (Railway usually redeploys automatically on a variable change).
3. Sign in again — you should land in the app with no bounce.

Fresh deployments get this automatically (the script wires the reference).

### The `api` service crash-loops on boot / login shows a server error

Check `railway logs --service api`. A failure on `CREATE EXTENSION IF NOT EXISTS
vector` means the Postgres plugin is not pgvector-capable. Provision a
pgvector-enabled Postgres (the local stack uses `pgvector/pgvector:pg16`) and
redeploy; the api runs migrations on boot and will not serve until they apply.
