...
/**
 * Manage the factory's own OAuth-connected sending mailboxes (Gmail / Microsoft).
 * Shared by the Settings › Sending email screen and the onboarding step, so both
 * present the same connect + status UI. Raw SMTP entry is intentionally absent —
 * mailboxes are connected only through the provider consent flow.
 */
export function SendingEmailManager({ showContinue = false }: { showContinue?: boolean }) {
  ...
  return (
    <div className="max-w-2xl">
      ...
      {/* Managed sending (Smartlead pool) — read-only explainer */}
      <div className="mt-4 rounded-xl bg-brand-50/60 p-6 ring-1 ring-brand-100">
        <p className="text-sm font-semibold text-brand-800">{t("managedTitle")}</p>
        <p className="mt-2 text-sm text-brand-900/80">{t("managedBody")}</p>
      </div>
      ...
      {loading ? (
        ...
        <ul className="mt-3 space-y-3">
          {accounts.map((a) => (
            <li key={a.id} className="rounded-xl bg-white p-4 shadow-sm ring-1 ring-black/5">
              ...
              {a.verification_status === "needs_reauth" && (
                <p className="mt-2 text-xs text-red-600">{t("reauthBanner", { email: a.email })}</p>
              )}
